...
class Scenario:
    def __init__(self, name: str, project: "Project"):
        ...
        self.inputs = {name: values.get(self.name) for name, values in self.project.inputs.items()}
        self.outputs = {name: values.get(self.name) for name, values in self.project.outputs.items()}
    ...
    def get_output(self, var_name: str):
        if var_name in self.outputs:
            return self.outputs[var_name]
        raise KeyError(f"Variable '{var_name}' not found in outputs.")

    # def has_inputs(self):
    #     return any(self.name in v["values"] for v in self.inputs.values())
