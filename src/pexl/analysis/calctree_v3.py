...
# ============================================================
# Full domain list found in the current uploaded schema files
# ============================================================
...
DEFAULT_DOMAIN_COLORS: Dict[str, str] = {
    ...
    "primray_energy": "#756bb1",  # as present in schema
    "project": "#969696",
    "tool": "#17becf",
    "usage": "#e377c2",
...
class SchemaCalcGraph:
    ...
    def build_group_graph(
        ...
        for gkey, members in group_members.items():
            ...
            if layer == "SIM" and len(members_sorted) == 1:
                display_text = icons[0] if icons else "∿"
            else:
            ...
            if layer == "SIM":
                ...
                sim_formula_annotation = "<br>".join(formulas)
            ...
            GG.add_node(
                ...
                annotation=sim_formula_annotation if layer == "SIM" else "",
    ...
    for node, data in G.nodes(data=True):
        ...
        hover = "<br>".join(
            [
                ...
                f"members: {'<br>'.join(data.get('members', []))}",
            ]
        )
...
if __name__ == "__main__":
    ...
    domain_colors = DEFAULT_DOMAIN_COLORS.copy()

    layer_layout = {
        "IN":  {"y": 0.0,   "dx": 3.0, "domain_gap": 2.2, "wave_amp": 1.35, "wave_period": 2.8, "wave_phase": 0.0},
        "SIM": {"y": -7.5,  "dx": 2.0, "domain_gap": 1.6, "wave_amp": 1.55, "wave_period": 1.2, "wave_phase": 0.8},
        "OUT": {"y": -14.5, "dx": 3.0, "domain_gap": 2.2, "wave_amp": 1.35, "wave_period": 2.8, "wave_phase": 1.6},
    }
    ...
    pos = calc.build_layered_positions(
        G,
        layout=layer_layout,
    )
